...
export default function ExperienceTimeline() {
  ...
  return (
    <section id="experience" className="w-full bg-background py-2 px-2 font-sans">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 border-x border-line">
        ...
        <div className="screen-line-top screen-line-bottom p-4 border-b border-line bg-muted/20">
          ...
          {experience.subText && (
            <p className="text-sm text-[#a392a3] leading-relaxed mt-1 font-sans">
              {experience.subText}
            </p>
          )}
        </div>
        ...
        <div className="p-4 space-y-4 border-b border-line bg-background">
          {experience.items?.map((item, idx) => (
            <div
              key={item.id || idx}
              className="cyber-card p-4 sm:p-6 rounded-xl border border-line space-y-3.5 shadow-xs transition-all hover:border-[#48b685] hover:bg-[#48b685]/5 transform hover:-translate-y-1 active:scale-[0.98]"
            >
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-line pb-3 font-sans">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-base text-foreground tracking-tight flex items-center gap-2 font-sans">
                      ...
                      {idx === 0 && (
                        <span className="text-[9px] font-mono font-extrabold text-[#48b685] bg-[#48b685]/10 border border-[#48b685]/30 px-2 py-0.5 rounded">
                          CURRENT_NODE
                        </span>
                      )}
                    </h3>
                ...
                <div className="flex flex-wrap items-center gap-2 text-[10px] font-mono self-start sm:self-auto">
              ...
              {/* Location Tag */}
              {item.location && (
                <div className="text-xs text-[#a392a3] flex items-center gap-1.5 font-sans">
              ...
              {/* Description */}
              <div className="text-sm text-foreground/90 leading-relaxed border-l-2 border-[#48b685]/50 pl-3.5 font-sans">
                <p>{item.description}</p>
              </div>

              {/* Tech Stack Pills at bottom */}
              {item.tags && item.tags.length > 0 && (
                <div className="pt-2 flex flex-wrap gap-2 border-t border-line font-mono">
